...
"""Итоговый отчёт по проездам KPP 4/KPP 5 из выгрузки Arvento.

Выгрузка может содержать только сообщения движения. Поэтому стоянка внутри
территории определяется как интервал более STOP_GAP_SECONDS между двумя
последовательными сообщениями движения, когда автомобиль находится внутри.
"""
...
# Если между двумя сообщениями движения прошло больше 10 минут,
# весь интервал считается стоянкой.
...
def near_gate(point: Point, gate: Gate) -> bool:
    ...
    return abs(across) <= GATE_MARGIN_M and abs(along) <= length / 2.0 + GATE_MARGIN_M
...
def crossing_fraction(p1: Point, p2: Point, gate: Gate) -> float:
    ...
    delta = a2 - a1
    return 0.5 if abs(delta) < 1e-12 else max(0.0, min(1.0, -a1 / delta))
...
def load_points(path: Path) -> tuple[list[Point], dict[str, int]]:
    ...
    for row in rows:
        ...
        if not plate or timestamp is None or lat is None or lon is None:
            ...
            continue
        points.append(Point(
            plate=plate, time=timestamp, lat=lat, lon=lon,
...
def build_gates(points: list[Point]) -> list[Gate]:
    kpp4_points = [point for point in points if KPP4_REGION.lower() in point.region.lower()]
    if not kpp4_points:
    ...
    gate4 = Gate(KPP4_REGION, (lat, lon - lon_delta / 2.0), (lat, lon + lon_delta / 2.0), len(kpp4_points))
    count5 = sum(KPP5_REGION.lower() in point.region.lower() for point in points)
    gate5 = Gate(KPP5_REGION, KPP5_POINT_1, KPP5_POINT_2, count5)
    return [gate4, gate5]


def add_inside_interval(seconds: float, totals: dict[str, float]) -> None:
    """Классифицирует интервал между сообщениями движения внутри территории."""
    if seconds <= 0:
        return
    totals["inside_seconds"] += seconds
    if seconds > STOP_GAP_SECONDS:
    ...
    for plate in sorted(grouped):
        ...
        for p1, p2 in zip(vehicle_points, vehicle_points[1:]):
            ...
            for gate in gates:
                ...
                if side2 != 0 and previous_side != 0 and side2 != previous_side:
                    ...
                    if elapsed <= MAX_CROSSING_WINDOW_SECONDS and (near_gate(p1, gate) or near_gate(p2, gate)):
                        kind = "Въезд" if previous_side == 1 and side2 == -1 else "Выезд"
                        candidates.append((crossing_fraction(p1, p2, gate), gate, kind))
                if side2 != 0:
            ...
            if candidate:
                ...
                if last_event_time is not None and (event_time - last_event_time).total_seconds() < EVENT_COOLDOWN_SECONDS:
                    candidate = None

            if candidate is None:
                if inside:
                    inside_km += distance_km
                    add_inside_interval(gap_seconds, time_totals)
                else:
            ...
            if kind == "Въезд":
                if inside:
                    ...
                    inside_km += after_km
                    add_inside_interval(after_seconds, time_totals)
                    inside = True
                    entries += 1
                ...
                if inside:
                    inside_km += before_km
                    outside_km += after_km
                    add_inside_interval(before_seconds, time_totals)
                    inside = False
                    exits += 1
                    last_event_time = event_time
                else:
        ...
        moving_percent = time_totals["moving_seconds"] / inside_seconds if inside_seconds > 0 else 0.0
        stopped_percent = time_totals["stopped_seconds"] / inside_seconds if inside_seconds > 0 else 0.0
        ...
        summaries.append({
            ...
            "stop_count": int(time_totals["stop_count"]),
...
def style_sheet(sheet) -> None:
    ...
    for column in range(1, sheet.max_column + 1):
        width = min(max(len(str(sheet.cell(row, column).value or "")) for row in range(1, min(sheet.max_row, 300) + 1)) + 2, 32)
...
def save_report(path: Path, summaries: list[dict[str, Any]]) -> None:
    ...
    headers = [
        "Госномер", "Начало периода", "Конец периода", "Состояние в начале", "Состояние в конце",
        "Въездов", "Выездов", "Пробег внутри, км", "Пробег снаружи, км", "Общий пробег, км",
        "Пробег внутри, %", "Пробег снаружи, %", "Время на территории",
        "Время в движении на территории", "Количество стоянок", "Время стоянки на территории",
        "Движение на территории, %", "Стоянка на территории, %",
    ]
    sheet.append(headers)
    for item in summaries:
        sheet.append([
            item["plate"], item["first_time"], item["last_time"], item["start_state"], item["end_state"],
            item["entries"], item["exits"], item["inside_km"], item["outside_km"], item["total_km"],
            item["inside_percent"], item["outside_percent"], item["inside_seconds"] / 86400.0,
            item["moving_seconds"] / 86400.0, item["stop_count"], item["stopped_seconds"] / 86400.0,
            item["moving_percent"], item["stopped_percent"],
        ])
    ...
    for row in sheet.iter_rows(min_row=2):
        ...
        row[10].number_format = row[11].number_format = "0.0%"
        for index in (12, 13, 15):
            row[index].number_format = "[h]:mm:ss"
        row[16].number_format = row[17].number_format = "0.0%"
...
def main() -> None:
    ...
    for gate in gates:
        print(f"{gate.name}: {gate.p1} -> {gate.p2}; точек зоны: {gate.sample_count}")
    summaries = analyze(points, gates)
    output = source.with_name(source.stem + "_КПП_отчет.xlsx")
    save_report(output, summaries)
    print(f"Готово: {output}")
    print(f"Автомобилей: {len(summaries)}; загружено точек: {stats['loaded']}; пропущено строк: {stats['skipped']}")
