...
from arvento_api_sync import HEADERS, build_params, fetch_chunk, parse_rows
        ...
        for row in rows:
            ...
            cur.execute(
                ...
                INSERT INTO gps_points (
                    ...
                    idling_duration, ignition_duration, source_hash
                ) VALUES (
                    %s,%s,%s,%s,%s,%s,
                ...
                (
                    ...
                    row.ignition_duration, source_hash(row),
                ),
            )
            if cur.fetchone():
                inserted += 1
    ...
    with psycopg.connect(database_url, row_factory=dict_row) as conn, requests.Session() as session:
        ...
        try:
            while current < end:
                ...
                try:
                    params = build_params(username, pin1, pin2, group, "", current.replace(tzinfo=None), chunk_end.replace(tzinfo=None), minute_dif)
                    ...
                    conn.commit()
                    print(f"{current:%Y-%m-%d %H:%M}–{chunk_end:%H:%M}: received={len(rows)} inserted={inserted}")
                    ...
                    conn.commit()
                    print(f"FAILED {current}–{chunk_end}: {exc}")
                current = chunk_end
