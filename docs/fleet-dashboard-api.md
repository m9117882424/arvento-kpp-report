...
Интерактивная документация формируется FastAPI из фактического контракта API:

```text
https://bdd.equippulse.com/docs
```

Портал защищает общую страницу Swagger своей Basic Auth. После входа:

1. раскройте раздел **Fleet API**;
2. нажмите **Authorize** в правом верхнем углу;
3. вставьте только исходный Fleet API token, без слова `Bearer`;
4. выберите endpoint, нажмите **Try it out**, заполните параметры и выполните запрос.

Swagger автоматически добавит заголовок `Authorization: Bearer <token>`. Токен
не следует сохранять в браузере, добавлять в URL, скриншоты или сообщения.

Машиночитаемая OpenAPI-схема доступна после Basic Auth:

```text
https://bdd.equippulse.com/openapi.json
```

Схема содержит Bearer security scheme, описания endpoint'ов и параметров,
модели всех успешных ответов, а также ответы `400`, `401`, `404`, `422` и `503`.
Она является основным контрактом для генерации клиентов и интеграционных тестов.
